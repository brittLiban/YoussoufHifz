 ...
 *
 * Shows 2 complete ayahs as the prompt.
 * Student recites the rest of the surah from there.
 * Reveal shows every remaining ayah to end of surah.
...
import type { AyahInfo } from '../src/types/quran';
...
interface QuizState {
  surahId: number;
  surahName: string;
  /** Index of the first prompt ayah (0-based within surah ayahs array) */
  promptStartIdx: number;
  /** All ayahs of this surah */
  ayahs: AyahInfo[];
}
...
export default function QuizScreen() {
  ...
  const generateQuiz = useCallback(() => {
    ...
    // Pick a starting index that leaves at least 1 ayah after the 2 prompt ayahs.
    // For surahs with exactly 2–3 ayahs, start at 0 so we still show something after.
    const maxStart = Math.max(0, ayahs.length - 3);
    const promptStartIdx = Math.floor(Math.random() * (maxStart + 1));
  ...
  const promptAyahs = quiz ? quiz.ayahs.slice(quiz.promptStartIdx, quiz.promptStartIdx + 2) : [];
  const remainingAyahs = quiz ? quiz.ayahs.slice(quiz.promptStartIdx + 2) : [];
    ...
    <View style={[styles.root, { backgroundColor: theme.bgPrimary }]}>
      <SafeAreaView edges={['top']}>
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={generateQuiz} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {quiz && (
          <View style={styles.surahLabel}>
            <Text variant="caption" secondary style={{ letterSpacing: 0.8 }}>
              {quiz.surahName.toUpperCase()}
              {' · '}AYAH {promptAyahs[0]?.ayahNum}–{promptAyahs[promptAyahs.length - 1]?.ayahNum}
            </Text>
            {surahEntry && (
        ...
        <Text variant="body" secondary style={styles.instruction}>
          Continue reciting from where these ayahs end:
        </Text>

        {/* Prompt — 2 complete ayahs */}
        {promptAyahs.length > 0 && (
          <Card style={styles.promptCard}>
            {promptAyahs.map((ayah, i) => (
              <View
                key={ayah.verseKey}
                style={i > 0 ? [styles.promptAyahSep, { borderTopColor: theme.border + '55' }] : undefined}
              >
                <RNText
                  style={{
            ...
            <Text variant="caption" secondary style={styles.continueHint}>
              ↓ continue reciting the rest of the surah
            </Text>
          </Card>
        )}
        ...
        {phase === 'prompt' && (
          <TouchableOpacity
            ...
            activeOpacity={0.8}
          >
            <Text variant="body" semiBold color="#F5F0E8">Reveal remaining ayahs</Text>
          </TouchableOpacity>
        )}

        {/* Revealed — rest of surah */}
        {(phase === 'revealed' || phase === 'rated') && (
          <Card style={styles.revealCard}>
            <Text variant="caption" secondary style={{ letterSpacing: 0.8, marginBottom: Spacing.md }}>
              REMAINING AYAHS — {remainingAyahs.length > 0 ? `${remainingAyahs.length} left` : 'end of surah'}
            </Text>
            {remainingAyahs.length === 0 ? (
              <Text variant="body" secondary style={{ textAlign: 'center', paddingVertical: Spacing.md }}>
                Those were the final ayahs of the surah.
              </Text>
            ) : (
              remainingAyahs.map((ayah, i) => (
                <View
                  key={ayah.verseKey}
                  style={[
                    styles.revealAyah,
...
const styles = StyleSheet.create({
  ...
  promptCard: { gap: Spacing.md, paddingBottom: Spacing.sm },
  promptAyahSep: {
